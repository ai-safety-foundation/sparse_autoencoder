...
def calc_capacities(
    features: Float[Tensor, "n_feats feat_dim"]
) -> Float[Tensor, " n_feats"]:
    """
    ...
    Example:
        ...
        >>> orthogonal_caps = calc_capacities(features)
        >>> assert torch.allclose(caps, torch.tensor([1, 1, 1]))
        >>> non_orthogonal_features = torch.tensor([[1, 0, 0], [1, 1, 0], [0, 0, 1]])
        >>> non_orthogonal_caps = calc_capacities(features)
        >>> assert torch.allclose(caps, torch.tensor([0.5, 0.8, 1]))
    ...
    sum_of_sq_dot = squared_dot_products.sum(dim=-1, keepdim=True)
