...
from sparse_autoencoder.activation_store.base_store import ActivationStoreBatch
...
class TestResizeListVectors:
    ...
    def test_resize_to_list_vectors(self, input_shape, expected_len, expected_shape):
    ...
    def test_resize_to_list_vectors_values(self):
...
class TestResizeSingleItemDimension:
    ...
    def test_resize_to_single_item_dimension(self, input_shape, expected_shape):
        """Check the resulting tensor shape."""
        input_tensor = torch.randn(input_shape)
        result = resize_to_single_item_dimension(ActivationStoreBatch(input_tensor))

        assert result.shape == expected_shape, f"Expected tensor shape {expected_shape}"

    def test_resize_to_single_item_dimension_values(self):
